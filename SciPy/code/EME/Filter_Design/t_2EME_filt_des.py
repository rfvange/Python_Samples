...
# M must be even so the length of the kernel is odd (formula can be adjusted for M odd).
# (i+0.5)-(M+1)/2 for i in range(M) -  M odd.

M = 500 # Gives a 164 Hz bandwidth
fc = 0.005 # Gives a 65 Hz bandwidth
#fc = 0.01 # Gives a 164 Hz bandwidth
#fc = 0.02 # Gives a 325 Hz bandwidth
#fc = 0.03 # Gives a 490 Hz bandwidth
...
for i in range(M):
  ...
  h[i] *= 0.42 - 0.5*np.cos(2*np.pi*i)/M + 0.08*np.cos(4*np.pi*i)/M
...
n=35 # 35?
complex_mult = [2*np.cos(n*2.0*i*np.pi/len(h)) for i in range(len(h))] # shifts from 0 to n
...
fout = open("flat_filt_kernel_65.out","w")
length = len(h_shift)
x2 = 100.0*h_shift
...
pylab.plot(x1,'k-',label="kernel1") # [0:70]
#pylab.plot(x2[0:20],'b-',label="kernel2")
