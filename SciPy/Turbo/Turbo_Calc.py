...
np.random.seed(2)
sigma = 0.7
Noise =  sigma * np.random.randn(3,7)    # the data array
# sigma * np.random.randn(rows,cols) + mu

Received_Data = Gold_Data + Noise
...
My_Data = np.zeros(3*7,dtype=float).reshape(3,7)

for i in range(len(Received_Data)):
  for j in range(len(Received_Data[0])):
    if Received_Data[i][j] <= 0:
      My_Data[i][j] = -1
    else:
      My_Data[i][j] = 1
